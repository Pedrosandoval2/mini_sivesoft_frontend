...
export function CodesModal({ isOpen, onClose, onAddCodes }) {
    ...
    const handleClearAll = () => {
        if (confirm("¿Estás seguro de que deseas eliminar todos los códigos?")) {
            reset({ codes: [{ value: "" }] })
            setTimeout(() => {
                inputRefs.current[0]?.focus()
            }, 0)
    ...
    const filledCodesCount = codesWatch.filter((code) => code.value?.trim().length > 0).length
                ...
                <div className="space-y-4">
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                        <p className="text-sm text-blue-900">
                            💡 <strong>Instrucciones:</strong> Escanea cada código con tu pistola lectora. Presiona Enter o espera a
                            que se agregue automáticamente el siguiente campo.
                        </p>
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-border">
                        ...
                        <div className="flex gap-2">
                            <Button onClick={handleAddInput} variant="outline" size="sm" className="bg-transparent">
